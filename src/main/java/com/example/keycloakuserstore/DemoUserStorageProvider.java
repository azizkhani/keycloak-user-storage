...
import com.example.keycloakuserstore.security.KhabirPasswordEncoder;
import com.example.keycloakuserstore.security.PasswordEncoder;
import com.example.keycloakuserstore.security.SSH512PasswordEncoder;
...
import static com.example.keycloakuserstore.DemoUserStorageProviderFactory.HASHING_VERIFY_API_URL;
...
public class DemoUserStorageProvider implements UserStorageProvider,
    ...
    public boolean isValid(RealmModel realm, UserModel user, CredentialInput credentialInput) {
        ...
        if (supportsCredentialType(credentialInput.getType())) {
            ...
            logger.info("credentialInput(" + credentialInput.getChallengeResponse() + ")");
            return password != null && validatePassword(credentialInput.getChallengeResponse(), password);
        } else {
    ...
    private boolean validatePassword(String rawPassword, String encodedPassword) {
        String apiUrl = model.getConfig().getFirst(HASHING_VERIFY_API_URL);
        PasswordEncoder passwordEncoder = new SSH512PasswordEncoder(apiUrl);
    ...
    public String getPassword(UserModel user) {
        ...
        if (user instanceof UserModel) {
            logger.info("user instanceof UserRepresentation");
